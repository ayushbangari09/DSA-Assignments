...
# Single Linked List to manage book records
class BookList:
    def __init__(self):
        self.head = None
    ...
    def searchBook(self, book_id):
        ...
        while current:
            if current.book_id == book_id:
                print(f" Found Book → ID: {current.book_id}, Title: {current.title}, Author: {current.author}, Status: {current.status}")
                return current
            current = current.next
        print(" Book not found!")
        return None
...
# Stack for managing transactions (issue/return)
